  ...
  <div id="thirdPayment">
    ...
    <Card dis-hover class="query_box">
      <div class="query_con">
        ...
        <div style="display: inline-block;float: right;" v-if="uidList.length > 0">
          <Button type="primary" @click="visible.sort = true">{{i18n_lang.newThirdPayment79_v_1_2_7_8}}</Button>
        </div>
      </div>
    ...
    <!-- 编辑 -->
    <Modal v-model="visible.edit" :title="i18n_lang.newThirdPayment94_v_1_2_7_8" :width="600" :styles="{top: '20px'}" @on-cancel="handleCloseModal">
    ...
    <!-- 批量排序 -->
    <Modal v-model="visible.sort" :title="`${i18n_lang.newThirdPayment79_v_1_2_7_8}`" :width="600" :styles="{top: '20px'}">
      <Form ref="sortForm" :model="sortInfo" :label-width="220" :rules="sortValidate">
        <FormItem :label="`${i18n_lang.newThirdPayment80_v_1_2_7_8}${uidList.length}${i18n_lang.newThirdPayment81_v_1_2_7_8}`" prop="sort">
          <Input v-model="sortInfo.sort" />
        </FormItem>
      </Form>
      <div slot="footer" class="footer">
        <Button @click="visible.sort = false">{{i18n_lang.newThirdPayment82_v_1_2_7_8}}</Button>
        <Button type="primary" :loading="loading.btn.modal" @click="SaveSort()">{{i18n_lang.newThirdPayment83_v_1_2_7_8}}</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      sortValidate: {
        sort: [{ required: true, message: `${i18n_lang.newThirdPayment84_v_1_2_7_8}` }]
      }
    }
  },
  created() {
