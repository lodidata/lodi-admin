...
export const dataList = function(Vue){
  ...
  return [
    ...
    },
    {
      ...
      minWidth: 80,
      render(h, { row }) {
        return <span class='red'>{row.money / 100}</span>
      },
      myExt: {
        label: 'money'
      }
    ...
    {
      title: `${i18n.getOutCheck152_v1_2_7_6}`,//提现手续费%
      align: 'center',
      minWidth: 80,
      render(h, { row }) {
        return <span>{row.fee ? `${row.fee}% ${row.fee_money ? "(" + row.fee_money / 100 + ")" : ""}` : '0%'}</span>
      },
      myExt: {
        label: 'fee'
      }
    ...
    {
      title: `${i18n.getOutCheck153_v1_2_7_6}`,//实际提现金额
      align: 'center',
      minWidth: 80,
      render(h, { row }) {
        return <span>{row.reality_money ? row.reality_money / 100 : ''}</span>
      },
      myExt: {
        label: 'real_money'
      }
