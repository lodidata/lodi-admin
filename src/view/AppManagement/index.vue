  ...
  <div>
    ...
    <Modal
      ...
      <Form
        ...
        :model="form"
        :rules="ruleValidate">
        <FormItem :label="`${i18n_lang.appManagement40_v1_2_6_3}`" prop="type">
          <Select v-model="form.type" @on-change="selectFn">
            <Option v-for="(item, index) in bagTypeList" :key="item.key" :value="item.key">{{ item.name }}</Option>
          </Select>
        </FormItem>
        <FormItem :label="`${i18n_lang.appManagement39_v1_2_6_3}`" prop="update_type">
          <RadioGroup v-model="form.update_type" size="large">
            <Radio label="1" v-if="form.type != 2">{{i18n_lang.appManagement36_v1_2_6_3}}</Radio>
            <Radio label="2">{{i18n_lang.appManagement37_v1_2_6_3}}</Radio>
          </RadioGroup>
        </FormItem>
        ...
        <FormItem :label="`${i18n_lang.appManagement2}`" prop="bag_url">
          <Upload :action="`*`" :before-upload="handleUpload" v-if="!form.bag_url">
          ...
          <div v-if="form.bag_url">
            ...
            <div>
              <Button type="success" @click="form.bag_url = ''">{{i18n_lang.appManagement35_change}}</Button>
            </div>
          </div>
        </FormItem>
