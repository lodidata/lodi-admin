  ...
  <div id="Member">
    ...
    <Card dis-hover class="query_box">
      <div class="query_con">
        ...
        <div class="item">
          <span>{{i18n_lang.memberManage118_v_1_2_5_6}}</span>
          <Input v-model="searchInfo.id" :placeholder="``" clearable style="width: 150px"/>
        </div>
        <!-- <div class="item">
    ...
    <Modal v-model="visible.modelBox" :title="i18n_lang.memberManage20">
      <Form ref="madalForm" :model="info" :label-width="80" :rules="ruleValidate1">
        <FormItem :label="`${i18n_lang.memberManage120_v1_2_5_2}`" prop="type">
          <RadioGroup v-model="info.type" @on-change="radioGroupFn">
            <Radio label="1">{{i18n_lang.memberManage121_v1_2_5_2}}</Radio>
            <Radio label="2">{{i18n_lang.memberManage122_v1_2_5_2}}</Radio>
          </RadioGroup>
        </FormItem>
        <FormItem :label="info.type == '2' ? `${i18n_lang.memberManage123_v1_2_5_2}` : i18n_lang.memberManage21" prop="agencyNum">
          <div>
            <Input
              v-model="info.agencyNum"
              :placeholder="info.type == '2' ? `${i18n_lang.memberManage124_v1_2_5_2}` : i18n_lang.memberManage22"
              style="width: 318px;"
            /><!-- @on-change="func" @on-focus="boxShow" -->
            <Button type="primary" @click="boxShow">{{i18n_lang.memberManage125_v1_2_5_2}}</Button>
          </div>
    ...
    <Modal v-model="visible.addModal" :loading="true" :title="i18n_lang.memberManage24">
      ...
      >
        ...
        <FormItem :label="`${i18n_lang.memberManage119_v1_2_5_2}`" prop="">
          <div>
            <Input
              v-model="addInfo.agent_id"
              @on-change="func"
              style="width: 318px;"
            /><!-- @on-focus="boxShow" -->
            <Button type="primary" @click="boxShow">{{i18n_lang.memberManage125_v1_2_5_2}}</Button>
          ...
          <ul v-if="agentList.length > 0 && visible.isShow" class="agentListBox">
            <li v-for="item in agentList" :key="item.name" @click="changeCurrentAgent(item, 'agent_id')">
              <span>{{ item.name }}</span>
            </li>
          </ul>
        </FormItem>
...
export default {
  data() {
    ...
    return {
      ...
      rake_back: [],
      ruleValidate1: {
        agencyNum: [{ required: true, validator: (rule, value, cb) => {
          if(this.info.type == '1' && !this.info.agencyNum){
            cb(new Error(i18n_lang.memberManage34))
          }else if(this.info.type == '2' && !this.info.agencyNum){
            cb(new Error(i18n_lang.memberManage124_v1_2_5_2))
          }else{
            cb()
          }
        }, trigger: 'blur'}]
  ...
  methods: {
    ...
    async getList(index = 1) {
      ...
      req.name = req.name ? req.name.toString().trim() : ''
      req.id = req.id ? req.id.toString().trim() : ''
      req.agent = req.agent ? req.agent.toString().trim() : ''
      req.truename = req.truename ? req.truename.toString().trim() : ''
      req.mobile = req.mobile ? req.mobile.toString().trim() : ''
      req.reg_ip = req.reg_ip ? req.reg_ip.toString().trim() : ''
      req.card = req.card ? req.card.toString().trim() : ''
      req.channel_id = req.channel_id ? req.channel_id.toString().trim() : ''
      req.game_account = req.game_account ? req.game_account.toString().trim() : ''
      let level = ''
    ...
    async createUser() {
      // TODO: 新建用户
      const { password, user_name, is_test, GAME, LIVE, SPORT, QP, CP, agent_id } = this.addInfo
      const user = {
        password,
        user_name,
        is_test,
    ...
    async searchAgent() {
      let req = {}
      if(this.info.type == '1'){
        const name = this.addInfo.agent_id ? this.addInfo.agent_id.replace(/\s+/g, '') : this.info.agencyNum.replace(/\s+/g, '')
        req = {name}
      }else{
        const number = this.info.agencyNum.replace(/\s+/g, '')
        req = {number}
      }

      const res = await this.$axios.request({
        url: this.info.type == '2' ? '/user/agent/channel' : '/user/agent/move',
        method: 'GET',
        params: { ...req }
      })
      if (res.status !== 200) {
    ...
    changeCurrentAgent(item, text) {
      this.selectAgent = item
      text ? this.addInfo.agent_id = item.name : this.info.agencyNum = item.number ? item.number : item.name
      this.addAgentId = item.id
      this.addAgentNumber = item.number ? item.number : ''
      this.agentList = []
    ...
    async changeAgent() {
      ...
      const res = await this.$axios.request({
        url: this.info.type == '2' ? `/user/agent/channel/${this.agentId}`  : `/user/agent/move/${this.agentId}`,
        method: 'PUT',
        data: {
          ...req
        }
