    ...
    <Card dis-hover class="query_box mb30">
      <div class="query_con">
        ...
        <div class="item">
          ...
          <Input v-model="initQuery.active_name" @on-keyup="activeNameSelect()" placeholder style="width:150px;"/>
          <ul class="itemList" v-if="active_name_list.length > 0">
            <li v-for="(item, index) in active_name_list" :key="index" @click="activeNameClick(item)">{{item.name}}</li>
          </ul>
        </div>
        <div class="item">
        ...
        <div class="item">
          <span>{{i18n_lang.joinList88_v1_2_7_8}}</span>
          <Select v-model="initQuery.recharge_type" style="width:150px">
            <Option :value="0">{{i18n_lang.joinList44}}</Option>
            <Option :value="1">{{i18n_lang.joinList89_v1_2_7_8}}</Option>
            <Option :value="2">{{i18n_lang.joinList90_v1_2_7_8}}</Option>
            <Option :value="3">{{i18n_lang.joinList91_v1_2_7_8}}</Option>
          </Select>
        </div>
        <div class="item">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 模糊搜索
      activeNameSelect(){
        this.active_name_list = []
        if(this.initQuery.active_name){
          this.applyList.forEach((item, index) => {
            if(item.name.indexOf(this.initQuery.active_name) != -1){
              this.active_name_list.push(item)
            }
          })
        }
      },
      // 选择活动名
      activeNameClick(item){
        this.initQuery.active_name = item.name
        this.active_name_list = []
      },
      // 选择日期
      selectTime(val) {
      ...
      async classList() {
        ...
        if(JSON.parse(parameter).Apply){
          this.initQuery = {
            ...
            active_id: JSON.parse(parameter).Apply.active_id ? JSON.parse(parameter).Apply.active_id : "",
            ...
            recharge_type: JSON.parse(parameter).Apply.recharge_type ? JSON.parse(parameter).Apply.recharge_type : ""
          }
      ...
      async getApplyUser() {
        const res = await this.$axios.request({
          url: '/active/apply/user/activeName',
          method: 'GET'
        })
        if (res.status !== 200) {
          this.$Message.error(res.message)
