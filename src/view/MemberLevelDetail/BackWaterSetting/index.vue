    ...
    >
      ...
      >
        <div style="padding-top: 10px;">
          ...
          ></Bingo>
          <SMALL
            v-if="item.type === 'SMALL' && currentGame === 'SMALL'"
            key="small"
            :list="item.list"
            :rebot-time="rebotTime"
            :type="item.type"
...
export default {
  ...
  methods: {
    async getData(id) {
      ...
      const minute = Number(res.data.rebot_time_minute)
      this.rebotTime = time
        ? `${time === 0 ? '00' : time < 10 ? `0${time}` : time}:${minute === 0 ? '00' : minute < 10 ? `0${minute}` : minute}:00`
        : `00:00:00`
    },
    updateData() {
      this.getData(this.$route.query.id)
