<template>
  <div id="commonMode">
    ...
    <Form ref="editForm" :model="editInfo" :label-width="120" :rules="ruleValidate" class="content">
      ...
      <div class="back-water-time-setting">
        <h1 class="title">
          <span>{{i18n_lang.memberLevelDetail89_v1_2_7_7}}</span>
          <span class="rule">{{i18n_lang.memberLevelDetail90_v1_2_7_7}}</span>
        </h1>
        <div class="max-rebate">
          <span>{{i18n_lang.memberLevelDetail91_v1_2_7_7}}</span>
          <Input v-model="dataSource[query.type_index].halls[query.hall_index].rebet_multiple" @on-keyup="rebateKeyUp()" style="width: 168px" :disabled="isEdit"></Input>
        </div>
        <div class="max-rebate" style="margin-top: 20px;">
          <span>{{i18n_lang.memberLevelDetail92_v1_2_8_0}}</span>
          <span><Switch v-model="dataSource[query.type_index].halls[query.hall_index].rebet_gt_zero_switch" :disabled="isEdit"></Switch></span>
          <span style="color: red;font-weight: normal;font-size: 12px;">{{i18n_lang.memberLevelDetail93_v1_2_8_0}}</span>
        </div>
      </div>
    </Form>
  </div>
...
export default {
  ...
  methods: {
    rebateKeyUp(){//日返输入整数
      this.dataSource[this.query.type_index].halls[this.query.hall_index].rebet_multiple = this.dataSource[this.query.type_index].halls[this.query.hall_index].rebet_multiple.replace(/\D/g,'')
    },
    init() {
    ...
    getPutArguments() {
      ...
      const obj = {
        type: 'rebate',
        menu: this.type,
        rebot_time: this.timeValue.split(':')[0].slice(0, 1) === 0 ? this.timeValue.split(':')[0].slice(1, 2) : this.timeValue.split(':')[0],
        rebot_time_minute: this.timeValue.split(':')[1].slice(0, 1) === 0 ? this.timeValue.split(':')[1].slice(1, 2) : this.timeValue.split(':')[1],
        rebot_way: {
        ...
        rebet_multiple: this.dataSource[this.query.type_index].halls[this.query.hall_index].rebet_multiple,
        status_switch: this.dataSource[this.query.type_index].halls[this.query.hall_index].status_switch,
        rebet_gt_zero_switch: this.dataSource[this.query.type_index].halls[this.query.hall_index].rebet_gt_zero_switch,
      }
      let opt = {}
      if (this.type === 'CP') {
