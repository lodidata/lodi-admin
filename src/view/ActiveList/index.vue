...
const ininData = {
  ...
  apply_times: '',        //可发起申请次数
  condition_recharge: 1  ,//申请条件-用户是否需要充值 1有 2没有
  condition_user_level: '',//用户等级限制(1,2,3)
  blacklist_url: '',
  rebet_multiple: '',//周返水倍数
  rebet_gt_zero_switch: true,
  link: '',//客服链接
...
export default {
  ...
  methods: {
    ...
    handleOpenModal(data) {
      ...
      if (data.template_id != 2 && data.template_id !== 4 && data.template_id != 7 && data.template_id != 17) {
        this.editInfo.withdraw_require_val = MathDiv(this.editInfo.withdraw_require_val, 100)
      }
      this.editInfo.send_bet_max = MathDiv(this.editInfo.send_bet_max, 100)
      this.editInfo.send_type = !this.editInfo.send_type || this.editInfo.send_type == 'null' ? '1' : this.editInfo.send_type
      ...
      if(data.template_id == 14){
        this.editInfo.rule = `${MathDiv(this.editInfo.rule, 100)}`
      }
    ...
    UpdateDate(e) {
      let res = this.activeList.filter(i => i.id == e)
      this.$set(res[0],'blacklist_url', '')
    }
  }
}
</script>
