<template>
  <div v-if="editInfo" id="AddActive">
    ...
    >
      ...
      <!-- 注册送 -->
      <!-- 新增活动和首次充值送300%除外 -->
      <FormItem v-if="curTab != 4 && curTab != 7 && curTab != 8 && curTab != 9 && curTab != 18" :label="i18n_lang.activeList16" prop="issue_mode"><!-- 赠送方式 -->
        <RadioGroup v-model="editInfo.issue_mode" size="large">
          <Radio label="manual" v-if="curTab != 14 && curTab != 17 && curTab != 18">{{i18n_lang.activeList17}}</Radio>
      ...
      <FormItem v-if="curTab == 4 && editInfo.state == 'contact'" :label="`${i18n_lang.activeList456_v1_2_7_8}`" prop="link">
        <Input v-model="editInfo.link" :placeholder="``"></Input>
      </FormItem>
      <FormItem v-if="curTab == 4 && editInfo.state == 'apply'" :label="i18n_lang.activeList448_v1_2_6_4">
        <div>
          <!-- <span>{{i18n_lang.activeList448_v1_2_6_4}}</span> -->
          <div style="width: 500px;overflow: hidden;">
            <!-- <Upload :action="``" :before-upload="handleUpload" style="float: left;">
              <Button size='small' type="primary" icon="ios-cloud-upload-outline">上传文件</Button>
            </Upload> -->
            <UploadImg v-model="editInfo.blacklist_url" :batchUrl2="true" :edit-type="editType" @reset="resetGift"></UploadImg>
          </div>
          <div>{{editInfo.blacklist_url ? editInfo.blacklist_url : ''}}</div>
        </div>
      </FormItem>
      <!-- 活动赠送次数 -->
      ...
      <FormItem v-if="curTab == 4 && editInfo.state == 'apply'" :label="`${i18n_lang.activeList394}`">
        <div><!-- {{`${i18n_lang.activeList395}：`}}<Switch v-model="editInfo.condition_recharge"></Switch> -->
          <span>{{`${i18n_lang.activeList395}：`}}</span>
          <span>
            <RadioGroup v-model="editInfo.condition_recharge" size="large">
              <Radio :label="0">{{`${i18n_lang.activeList448_v1_2_5_6}`}}</Radio>
              <Radio :label="1">{{`${i18n_lang.activeList449_v1_2_5_6}`}}</Radio>
              <Radio :label="2">{{`${i18n_lang.activeList450_v1_2_5_6}`}}</Radio>
              <Radio :label="3">{{`${i18n_lang.activeList451_v1_2_5_6}`}}</Radio>
              <Radio :label="4">{{`${i18n_lang.activeList452_v1_2_5_6}`}}</Radio>
            </RadioGroup>
          </span>
        </div>
        <div>{{`${i18n_lang.activeList396}：`}}<Switch v-model="isLevel"></Switch></div>
      </FormItem>
      <!-- 会员等级 -->
      ...
      <!-- 注册送注册送/实名认证 -->
      <!-- 每日首冲/新人首冲 -->
      <FormItem v-if="curTab === 2 || curTab === 3 || curTab == 11 || curTab == 17 || curTab == 18" :label="i18n_lang.activeList21" prop="send_type"><!-- 赠送金额 -->
      ...
      <FormItem v-if="curTab === 2 || curTab === 3 || curTab == 11 || curTab == 17" :label="i18n_lang.activeList24"><!-- 充值优惠 -->
        <Row>
      ...
      <FormItem v-if="(editInfo.send_type === '2' && curTab != 7 && curTab != 18) || curTab == 11" prop="send_max" :label="i18n_lang.activeList30"><!-- 赠送上限 -->
      ...
      <FormItem v-if="curTab === 2 || curTab === 3 || curTab == 11 || curTab == 17" :label="i18n_lang.activeList32" prop="vender_type"><!-- 充值方式 -->
        <RadioGroup v-model="editInfo.vender_type" size="large">
          <Radio label="1" v-if="curTab != 17">{{i18n_lang.activeList33}}</Radio>
          <Radio label="2">{{curTab == 17 ? `paymaya` : i18n_lang.activeList34}}</Radio>
          <Radio label="3" v-if="curTab != 17">{{i18n_lang.activeList35}}</Radio>
        </RadioGroup>
      </FormItem>
      <!-- 每日首冲/新人首冲 -->
      <!-- 新增活动和首次充值送300%除外 --><!-- 提现打码量% -->
      <FormItem v-if="curTab != 4 && curTab != 7 && curTab != 8 && curTab != 9 && curTab != 10 && curTab != 11 && curTab != 13 && curTab != 18" :label="i18n_lang.activeList36" prop="withdraw_require_val">
      ...
      <h1 class="title" v-if="curTab == 8 || curTab == 9">
        <span>{{i18n_lang.activeList450_v1_2_7_7}}</span>
        <span class="rule">{{i18n_lang.activeList451_v1_2_7_7}}</span>
      </h1>
      <FormItem v-if="curTab == 8 || curTab == 9" :label="curTab == 9 ? `${i18n_lang.activeList452_v1_2_7_7} ≥`  : `${i18n_lang.activeList453_v1_2_7_7} ≥`">
        <Input v-model="editInfo.rebet_multiple" :placeholder="curTab == 9 ? `${i18n_lang.activeList454_v1_2_7_7}` : `${i18n_lang.activeList455_v1_2_7_7}`" @on-keyup="editInfo.rebet_multiple = editInfo.rebet_multiple.replace(/\D/g,'')"></Input>
      </FormItem>
      <FormItem v-if="curTab == 8 || curTab == 9" :label="`${i18n_lang.activeList456_v1_2_8_0}`">
        <Switch v-model="editInfo.rebet_gt_zero_switch" :disabled="isEdit"></Switch>
      </FormItem>
      <h1 class="title" v-if="curTab == 8 || curTab == 9">
      ...
      <!-- 赠送金额配置 -->
      <FormItem v-if="curTab == 18" prop="rechargeNumberArrar1">
        <div>
          <span><i style="color: red;">*</i>{{i18n_lang.activeList457_v1_2_7_8}}</span>
          <Input v-model="recharge_number[0].number1" @on-keyup="clearNoNum('recharge_number[0]', 'number1', 0)" style="width: 88px;"></Input>
          <span>{{i18n_lang.activeList458_v1_2_7_8}}</span>
          <Input v-model="recharge_number[0].number2" style="width: 88px;"></Input>
          <span>{{i18n_lang.activeList459_v1_2_7_8}}</span>
          <Input v-model="recharge_number[0].number3" style="width: 88px;"></Input><span>{{editInfo.send_type == 2 ? "%" : ''}}</span>
          <span>{{i18n_lang.activeList460_v1_2_7_8}}</span>
          <Input v-model="recharge_number[0].number4" style="width: 88px;"></Input><span>{{editInfo.send_type == 2 ? "%" : ''}}</span>
        </div>
      </FormItem>
      <FormItem v-if="curTab == 18" prop="rechargeNumberArrar2">
        <div>
          <span><i style="color: red;">*</i>{{i18n_lang.activeList461_v1_2_7_8}}</span>
          <Input v-model="recharge_number[1].number1" style="width: 88px;"></Input>
          <span>{{i18n_lang.activeList458_v1_2_7_8}}</span>
          <Input v-model="recharge_number[1].number2" style="width: 88px;"></Input>
          <span>{{i18n_lang.activeList459_v1_2_7_8}}</span>
          <Input v-model="recharge_number[1].number3" style="width: 88px;"></Input><span>{{editInfo.send_type == 2 ? "%" : ''}}</span>
          <span>{{i18n_lang.activeList460_v1_2_7_8}}</span>
          <Input v-model="recharge_number[1].number4" style="width: 88px;"></Input><span>{{editInfo.send_type == 2 ? "%" : ''}}</span>
        </div>
      </FormItem>
      <FormItem v-if="curTab == 18" prop="rechargeNumberArrar3">
        <div>
          <span><i style="color: red;">*</i>{{i18n_lang.activeList462_v1_2_7_8}}</span>
          <Input v-model="recharge_number[2].number1" style="width: 88px;"></Input>
          <span>{{i18n_lang.activeList458_v1_2_7_8}}</span>
          <Input v-model="recharge_number[2].number2" style="width: 88px;"></Input>
          <span>{{i18n_lang.activeList459_v1_2_7_8}}</span>
          <Input v-model="recharge_number[2].number3" style="width: 88px;"></Input><span>{{editInfo.send_type == 2 ? "%" : ''}}</span>
          <span>{{i18n_lang.activeList460_v1_2_7_8}}</span>
          <Input v-model="recharge_number[2].number4" style="width: 88px;"></Input><span>{{editInfo.send_type == 2 ? "%" : ''}}</span>
        </div>
      </FormItem>
      <!-- <FormItem :label="``">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      ruleValidate: {
        ...
        rechargeNumberArrar1:[
          {
            required: true,
            validator: (rule, value, cb) => {
              if (!this.recharge_number[0].number1 || !this.recharge_number[0].number2 || !this.recharge_number[0].number3 || !this.recharge_number[0].number4) {
                return cb(`${i18n_lang.activeList463_v1_2_7_8}`)
              }else if(Number(this.recharge_number[0].number2) <= Number(this.recharge_number[0].number1)){
                return cb(`${i18n_lang.activeList464_v1_2_7_8}`)
              }else {
                return cb()
              }
            }
        ...
        rechargeNumberArrar2:[
          {
            required: true,
            validator: (rule, value, cb) => {
              if(Number(this.recharge_number[1].number2) <= Number(this.recharge_number[1].number1)){
                return cb(`${i18n_lang.activeList464_v1_2_7_8}`)
              }else {
                return cb()
              }
        ...
        rechargeNumberArrar3:[
          {
            required: true,
            validator: (rule, value, cb) => {
              if(Number(this.recharge_number[2].number2) <= Number(this.recharge_number[2].number1)){
                return cb(`${i18n_lang.activeList464_v1_2_7_8}`)
              }else {
                return cb()
              }
      ...
      if(oldName){
        that.backwaterList.forEach((item) => {
          if(item.game_menu_id == oldName){
            ...
            // item.rebet_multiple = that.rebet_multiple
            item.data = {
            ...
            that.backwaterList.push({
              ...
              type: that.moneyType,
              // rebet_multiple: that.rebet_multiple,
              data: {
                status: that.childMoneyType,
                value: that.moneyRange
              }
      ...
      if(that.backwaterList.length){
        ...
        that.backwaterList.forEach( item => {
          if(item.game_menu_id == newName) {
            ...
            // that.rebet_multiple = item.rebet_multiple
          }else{
  ...
  methods: {
    // 输入数字
    clearNoNum(object, value, number){
      // this.recharge_number[0].number1 = this.recharge_number[0].number1.replace(/[^\d^\.]+/g, "")
      // this.recharge_number[0].number1 = this.recharge_number[0].number1.replace(/^0+(\d)/, "$1")
      // this.recharge_number[0].number1 = this.recharge_number[0].number1.replace(/^\./, "0.")
      // this.recharge_number[0].number1 = this.recharge_number[0].number1.match(/^\d*(\.?\d{0,2})/g)[0] || "";
      // ^(([1-9]{1}\d*)|(0{1}))(\.\d{2})$
    ...
    async resetGift() {
      const res = await this.$axios.request({
        url: `/active/clear/${this.editInfo.id}`,
        method: 'PUT'
      })
      if (res.status !== 200) {
        this.$Message.error(res.message)
      ...
      this.editInfo.blacklist_url = ''
      this.$emit('update', this.editInfo.id)
      this.$Message.success(this.i18n_lang.activeList451_v1_2_6_4)
    },
    handleUpload(file){
      this.gileData.fileName = file.name
      this.gileData.formData = new FormData()
      this.gileData.formData.append('file', file)
      this.gileData.formData.append('name', file.name)
    },
    // 周月流水 start
    ...
    changeTab(id) {
      ...
      if(id == 7 && this.editType === 'edit'){
        this.discounts = {
          discounts0: MathDiv(this.editInfo.rule.split('|')[0].split(',')[0], 100),
          discounts1: MathDiv(this.editInfo.rule.split('|')[0].split(',')[1], 100),
          discounts2: MathDiv(this.editInfo.rule.split('|')[0].split(',')[2], 100),
          discounts3: MathDiv(this.editInfo.rule.split('|')[1].split(',')[0], 100),
          discounts4: MathDiv(this.editInfo.rule.split('|')[1].split(',')[1], 100),
          discounts5: MathDiv(this.editInfo.rule.split('|')[1].split(',')[2], 100),
          discounts6: MathDiv(this.editInfo.rule.split('|')[2].split(',')[0], 100),
          discounts7: MathDiv(this.editInfo.rule.split('|')[2].split(',')[1], 100),
          discounts8: MathDiv(this.editInfo.rule.split('|')[2].split(',')[2], 100),
          discounts9: ''
        }
      ...
      if(id==8 || id==9){
        ...
        if(rule[0]){
          ...
          // this.rebet_multiple = rule[0].rebet_multiple
        ...
        if(rule.length){
          ...
          arrarList.forEach((item, index) => {
            ...
            obj.type = item.type
            // obj.rebet_multiple = item.rebet_multiple
        ...
        // this.rebet_multiple = rule && rule.rebet_multiple ? rule.rebet_multiple :  ''
      ...
      this.editInfo.vender_type = id==17 ? '2' : '1'
      if(id==18){
        if(this.editType === 'add'){
        ...
        if(this.editType === 'edit'){
          this.recharge_number = [
            {
              number1: this.editInfo.rule.split(';')[0] && this.editInfo.rule.split(';')[0].split(',')[0] ? this.editInfo.rule.split(';')[0].split(',')[0] / 100 : '',
              number2: this.editInfo.rule.split(';')[0] && this.editInfo.rule.split(';')[0].split(',')[1] ? this.editInfo.rule.split(';')[0].split(',')[1] / 100 : '',
              number3: this.editInfo.rule.split(';')[0] && this.editInfo.rule.split(';')[0].split(',')[2] ? this.editInfo.rule.split(';')[0].split(',')[2] / 100 : '',
              number4: this.editInfo.rule.split(';')[0] && this.editInfo.rule.split(';')[0].split(',')[3] ? this.editInfo.rule.split(';')[0].split(',')[3] / 100 : ''
            },
            {
              number1: this.editInfo.rule.split(';')[1] && this.editInfo.rule.split(';')[1].split(',')[0] ? this.editInfo.rule.split(';')[1].split(',')[0] / 100 : '',
              number2: this.editInfo.rule.split(';')[1] && this.editInfo.rule.split(';')[1].split(',')[1] ? this.editInfo.rule.split(';')[1].split(',')[1] / 100 : '',
              number3: this.editInfo.rule.split(';')[1] && this.editInfo.rule.split(';')[1].split(',')[2] ? this.editInfo.rule.split(';')[1].split(',')[2] / 100 : '',
              number4: this.editInfo.rule.split(';')[1] && this.editInfo.rule.split(';')[1].split(',')[3] ? this.editInfo.rule.split(';')[1].split(',')[3] / 100 : ''
            },
            {
              number1: this.editInfo.rule.split(';')[2] && this.editInfo.rule.split(';')[2].split(',')[0] ? this.editInfo.rule.split(';')[2].split(',')[0] / 100 : '',
              number2: this.editInfo.rule.split(';')[2] && this.editInfo.rule.split(';')[2].split(',')[1] ? this.editInfo.rule.split(';')[2].split(',')[1] / 100 : '',
              number3: this.editInfo.rule.split(';')[2] && this.editInfo.rule.split(';')[2].split(',')[2] ? this.editInfo.rule.split(';')[2].split(',')[2] / 100 : '',
              number4: this.editInfo.rule.split(';')[2] && this.editInfo.rule.split(';')[2].split(',')[3] ? this.editInfo.rule.split(';')[2].split(',')[3] / 100 : ''
            },
          ]
        }
      }
    ...
    async saveActive() {
      ...
      this.$refs['editInfo'].validate(async valid => {
        if (valid) {
          ...
          obj.begin_time = this.$dateTimeFormat(obj.begin_time)
          obj.end_time = this.$dateTimeFormat(obj.end_time)
          ...
          if (this.curTab === 2 || this.curTab === 3 || this.curTab === 11 || this.curTab == 17) {
            if (obj.send_type === '2') {
          ...
          if (this.curTab != 2 && this.curTab !== 4 && this.curTab != 7 && this.curTab != 17) {
          ...
          if (this.curTab == 7) {
            ...
            let minMoney = MathMul(this.minMoney, 100)
          ...
          if(this.curTab == 8 || this.curTab == 9){
            ...
            if(this.backwaterList.length == 0){
              this.backwaterList.push({
                ...
                // rebet_multiple: this.rebet_multiple
              })
            }
            this.backwaterList.forEach((item, index) => {
              if(item.game_menu_id == this.currentGameId){
                ...
                // item.rebet_multiple = this.rebet_multiple
              }else{
                this.backwaterList.push({
                  game_menu_id: this.currentGameId,
                  type: this.moneyType,
          ...
          if(this.curTab == 18){
            obj.issue_mode = 'auto'
            obj.vender_type = 1
            obj.rule=`${this.recharge_number[0].number1 * 100},${this.recharge_number[0].number2 * 100},${this.recharge_number[0].number3 * 100},${this.recharge_number[0].number4 * 100};${this.recharge_number[1].number1 * 100},${this.recharge_number[1].number2 * 100},${this.recharge_number[1].number3 * 100},${this.recharge_number[1].number4 * 100};${this.recharge_number[2].number1 * 100},${this.recharge_number[2].number2 * 100},${this.recharge_number[2].number3 * 100},${this.recharge_number[2].number4 * 100}`
          }
          // console.log(obj, 'obj-------');
          if(this.curTab != 4){
            delete obj.apply_times
          }
          delete obj.rechargeNumberArrar1
          delete obj.rechargeNumberArrar2
          delete obj.rechargeNumberArrar3
