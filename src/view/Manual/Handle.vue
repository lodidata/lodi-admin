  ...
  <div id="Handle">
    ...
    <div v-if="visible.info" class="tab_box">
      ...
      <div class="content">
        <Row>
          ...
          <div class="item">
            ...
            <span class="detail">{{ dataSource.total_balance / 100 }}</span>
          </div>
          </Col>
          <Col span="6">
          <div class="item">
        ...
        <div class="content">
          <Row>
            ...
            <div class="item">
              ...
              <span class="detail">{{ dataSource.primary[0].balance / 100 }}</span>
            </div>
            </Col>
            <Col span="4">
            <div class="item">
              <p>{{i18n_lang.manual23}}</p>
              <span class="detail">{{ dataSource.dml.canMoney / 100 }}</span>
            </div>
            </Col>
            <Col span="4">
            <div class="item">
              <p>{{i18n_lang.manual140_v_1_2_5_4}}</p>
              <span class="detail">{{ dataSource.direct_balance / 100 }}</span>
            </div>
            </Col>
            <Col span="4">
            <div class="item">
              <p>{{i18n_lang.manual24}}</p>
              <span>{{ dataSource.dml.codes / 100 }}</span>
            </div>
            </Col>
            <Col span="4">
            <div class="item">
              <p>{{i18n_lang.manual25}}</p>
              <span>{{ dataSource.dml.factCode / 100 }}</span>
            </div>
            </Col>
          </Row>
        </div>
        ...
        <Col v-for="(it,i) in dataSource.secondary" :key="i" span="12">
          ...
          <div class="content">
            <div class="item">
              ...
              <span class="detail">{{ it.balance / 100 }}</span>
              <Button
                type="primary"
      ...
      <Row v-if="allRoutes.indexOf('396') > -1">
        ...
        <div>
          <Button type="primary" @click="addhand(1)">{{i18n_lang.manual100}}</Button>
          <Button type="primary" @click="addhand(2)">{{i18n_lang.manual134_v_1_2_5_4}}</Button>
          <Button type="primary" @click="addhand(3)">{{i18n_lang.manual135_v_1_2_5_4}}</Button>
        </div>
      </Row>
      ...
      <Row v-if="allRoutes.indexOf('397') > -1 || allRoutes.indexOf('398') > -1">
        ...
        <div class="content">
          <Col span="6">
            <div class="item">
              <p>{{i18n_lang.manual22}}</p>
              <span class="detail">{{ dataSource.share_balance ? dataSource.share_balance / 100 : 0 }}</span>
            </div>
          </Col>
        </div>
      </Row>
    ...
    <!-- 手动发放 -->
    <Modal v-model="visible.hand" :title="`${manualTitle}`">
      <Form :model="form.handInfo">
        <FormItem :label="`${i18n_lang.manual102}`">
          <Input v-model="dataSource.username" disabled></Input>
        </FormItem>
        <FormItem :label="`${manualMoney}`">
          <Input v-model="form.handInfo.discount"></Input>
        </FormItem>
        <FormItem :label="`${i18n_lang.manual104}`" v-if="manualType != 3">
          <Input v-model="form.handInfo.play_code"></Input>
        </FormItem>
        <FormItem :label="i18n_lang.manual54">
          <Input v-model="form.handInfo.memo" type="textarea"></Input>
        </FormItem>
        <FormItem :label="`${i18n_lang.manual108_v_1_2_4_6}`" prop="pin_pw" v-if="dataSource.need_pin_password && manualType == 2">
          <Input v-model="form.handInfo.pin_pw"></Input>
        </FormItem>
      </Form>
      <div slot="footer">
...
export default {
  ...
  methods: {
    ...
    addhand(number){
      if(number == 1){
        this.manualTitle = this.i18n_lang.manual101
        this.manualMoney = this.i18n_lang.manual103
      }
      if(number == 2){
        this.manualTitle = this.i18n_lang.manual136_v_1_2_5_4
        this.manualMoney = this.i18n_lang.manual137_v_1_2_5_4
      }
      if(number == 3){
        this.manualTitle = this.i18n_lang.manual138_v_1_2_5_4
        this.manualMoney = this.i18n_lang.manual139_v_1_2_5_4
      }
      this.form.handInfo = { ...handInfo }
      this.manualType = number
    ...
    async setHand(){
      ...
      // if(this.manualType == 3 && req.discount * 100 > this.dataSource.direct_balance){
      //   this.loading.btn = false
      //   this.$Message.error('123')
      //   return
      // }
      this.manualType == 1 ? req.discount = MathMul(req.discount, 100) : req.amount = MathMul(req.discount, 100)
      req.play_code = MathMul(req.play_code, 100)
      req.uid = this.dataSource.uid
      this.manualType != 1 ? delete req.discount : ''
      this.manualType != 2 ? delete req.pin_pw : ''
      let url = ``
      if(this.manualType == 1){
        url = `/cash/manual/coupon`
      }
      if(this.manualType == 2){
        url = `/cash/manual/direct/increase`
      }
      if(this.manualType == 3){
        url = `/cash/manual/direct/decrease`
      }
      const res = await this.$axios.request({
        url: `${url}`,
        method: 'POST',
