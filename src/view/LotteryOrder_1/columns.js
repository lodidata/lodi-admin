...
export default function(Vue) {
  ...
  return [
    ...
    },
    {
      ...
      render: (h, { row }) => {
        return (
          <span class={row.tags === 4 || row.tags === 7 || row.tags === i18n.lotteryOrder99 ? 'red' : 'user_link'} on-click={_ => Vue.handleUserDetail(row)}>
            {row.user_name}
          </span>
        )
      }
    ...
    },
    {
      ...
      render: (h, { row }) => {
        return <span>{`${row.play_name}`}</span>
    ...
    },
    {
      ...
      render(h, { row }) {
        return (
          <div>{lottTranslator.translateBallContentText(row.play_id, row.play_number)}</div>
          // <div>{ row.play_number }</div>
          // <div>
          //   {row.play_number.map((x, i) => (
    ...
    },
    {
      ...
      minWidth: 70,
      render(h, { row }) {
        return <span>{MathDiv(row.one_money, 100)}</span>
      },
      myExt: {
        label: 'one_money'
      }
    },
    {
      ...
      minWidth: 70,
      render(h, { row }) {
        return <span>{MathDiv(row.pay_money, 100)}</span>
      },
      myExt: {
        label: 'pay_money'
      }
    ...
    },
    {
      ...
      minWidth: 70,
      render(h, { row }) {
        return <span>{MathDiv(row.p_money, 100)}</span>
      },
      myExt: {
        label: 'p_money'
      }
    },
    {
      ...
      minWidth: 70,
      render(h, { row }) {
        return <span>{MathDiv(row.lose_earn, 100)}</span>
      },
      myExt: {
        label: 'lose_earn'
      }
    ...
    //   },
    //   render(h, { row }) {
    //     return row.state2 === i18n.lotteryOrder95 ? (
    //       <Poptip
    //         confirm
