...
export default function(Vue) {
  ...
  return [
    ...
    },
    {
      ...
      align: 'center',
      render(h, { row }) {
        return row.pay_money === '' ? '' : <span> {row.pay_money / 100}</span>
      }
    },
    {
    ...
    },
    {
      ...
      align: 'center',
      render(h, { row }) {
        return row.send_money === '' ? '' : <span> {Number(row.send_money) / 100}</span>
      }
    },
    {
    ...
    },
    {
      ...
      render(h, { row }) {
        return row.min_prize_once_money === '' ? (
          ...
          <span> {Number(row.min_prize_once_money) / 100}</span>
        )
      }
    },
    {
      ...
      render(h, { row }) {
        return row.max_prize_once_money === '' ? (
          ...
          <span> {Number(row.max_prize_once_money) / 100}</span>
        )
      }
    },
    {
    ...
    },
    {
      ...
      render(h, { row }) {
        return row.jackpot === '' ? '' : <span>{row.jackpot / 100}</span>
      }
    },
    {
