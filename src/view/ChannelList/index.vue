  ...
  <div>
    ...
    <Modal v-model="visible.handle" :title="`${handleInfo.id ? i18n_lang.channelList6 : i18n_lang.channelList5}${i18n_lang.channelList7}`">
      <Form
        ...
        <FormItem :label="`${i18n_lang.channelList34_ChannelAuthority}`" prop="admin_user">
          <Select v-model="handleInfo.admin_user" multiple @on-change="getAdminUser($event)" placeholder=" "
            style="min-width: 149px;">
            <Option :key="''" :value="''">{{`${i18n_lang.channelList35_ChannelAuthority}`}}</Option>
            <Option v-for="item in adminUserList" :key="item.id + ''" :value="item.id + ''">{{ item.username }}</Option>
          </Select>
        </FormItem>
      </Form>
      <div slot="footer">
        ...
        ruleValidate: {
          ...
          admin_user: [{
            ...
            validator: (rule, value, cb) => {
              if (!this.handleInfo.admin_user.length) {
                return cb(`${i18n_lang.channelList36_ChannelAuthority}`)
              } else {
                return cb()
              }
            },
    ...
    methods: {
      ...
      handle(row){
        if(row){
          this.handleInfo = {
            ...
            url: row.url,
            remark: row.remark,
            admin_user: row.admin_user ? row.admin_user.split(',') : []
          }
          this.urlList = []
          if(row.url){
      ...
      getAdminUser(event){
        event.forEach((item, index) => {
          if (item === '') {
            this.handleInfo.admin_user = []
            this.adminUserList.forEach((items, indexs) => {
              this.handleInfo.admin_user.push(items.id + '')
            })
          }
        })
      },
      ...
      async handleSave(){
        this.$refs['handleInfo'].validate(async valid => {
          if (valid) {
            ...
            let admin_user = ''
            req.admin_user.forEach((item, index) => {
              admin_user+= index>0 ? `,${item}` : `${item}`
            })
