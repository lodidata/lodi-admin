      ...
      <li v-if="active.infoTab=='info'" class="info-con">
        <template v-if="Object.keys(userInfo).length > 1 && !loading.page">
          ...
          <p>
            ...
            <span>{{ MathDiv(userInfo.deposit_withdraw.deposit_sum,100) }}{{i18n_lang.serviceWorkbench75}}</span>
          </p>
          <p>
            <span>{{i18n_lang.serviceWorkbench77}}</span>
            <span>{{ MathDiv(userInfo.deposit_withdraw.withdraw_sum,100) }}{{i18n_lang.serviceWorkbench75}}</span>
          </p>
          <p>
            <span>{{i18n_lang.serviceWorkbench78}}</span>
            <span>{{ MathDiv(userInfo.earn_loss.earn_loss,100) }}{{i18n_lang.serviceWorkbench75}}</span>
          </p>
          <p>
            <span>{{i18n_lang.serviceWorkbench79}}</span>
            <span>{{ MathDiv(userInfo.dml_info.free_money,100) }}{{i18n_lang.serviceWorkbench75}}</span>
          </p>
          <p>
            <span>{{i18n_lang.serviceWorkbench80}}</span>
            <span>{{ MathDiv(userInfo.dml_info.require_bet,100) }}{{i18n_lang.serviceWorkbench75}}</span>
          </p>
          <p>
            <span>{{i18n_lang.serviceWorkbench81}}</span>
            <span>{{ MathDiv(userInfo.dml_info.actual_bet,100) }}{{i18n_lang.serviceWorkbench75}}</span>
          </p>
          <!-- <p>
            <span>访问来源:</span>
