  ...
  <div id="OnlineIncome">
      ...
      <div class="query_con">
        ...
        <div class="item">
          ...
          <Select v-model="query.status" style="width:100px">
            ...
            <Option value="canceled">{{i18n_lang.payManage181_canceled}}</Option>
          </Select>
        </div>
        ...
        <div class="item">
          <span>{{i18n_lang.payManage90}}</span>
          <Select v-model="query.payment_id" style="width:100px">
            <Option value>{{i18n_lang.payManage71}}</Option>
            <Option v-for="(it,i) in paychannelList" :key="i" :value="it.id">{{ it.name }}</Option>
          </Select>
        </div>
        ...
        <div class="item">
          <span>{{i18n_lang.payManage180_channel_no}}</span>
          <Input v-model="query.channel_no" placeholder style="width: 100px"></Input>
        </div>
      </div>
    ...
    <Card dis-hover class="query_box">
      <div class="statistics">
        ...
        <div>
          <div>{{i18n_lang.payManage182_v1_2_7_8}}{{totalInfo.current_success_rate ? totalInfo.current_success_rate : ``}}</div>
          <!-- <div>{{i18n_lang.payManage169}}{{totalInfo.all_account_count ? Number(totalInfo.all_account_count) / 100 : 0}}</div> -->
        </div>
      </div>
    </Card>
...
export default {
  ...
  methods: {
    ...
    async getQuery() {
      ...
      const paychannel = this.getCashPaychannel()
      Promise.all([paychannel, third, level]).then(res => {
        this.loading.page--
      })
    },
    ...
    async handleDetail(row) {
      ...
      if (res.status !== 200) {
        ...
        this.detailData.coupon_money = MathDiv(this.detailData.coupon_money, 100)
        this.detailData.withdraw_bet = MathDiv(this.detailData.withdraw_bet, 100)
        this.detailData.money = MathDiv(this.detailData.money, 100)
        this.detailData.total_deposit_money = MathDiv(this.detailData.total_deposit_money, 100)
