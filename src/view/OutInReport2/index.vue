<template>
  <div id="OutInReport">
        ...
        <div class="query_con">
          <div class="item">
            ...
            <!-- <DatePicker
              ...
              style="width: 200px"
            ></DatePicker> -->
            <DatePicker v-model="query.start_time" type="datetime" placeholder style="width: 180px" :options="startTimeOpt" :editable="false"></DatePicker>
            <span>-</span>
            <DatePicker v-model="query.end_time" type="datetime" placeholder style="width: 180px" :options="endTimeOpt" :editable="false"></DatePicker>
          </div>
        </div>
    ...
    <div v-if="detail.visible && visible.channel == 3">
      ...
      <span>{{ detail.query.name }}</span>
      <span style="margin-left: 20px;" v-if="is_deduct">{{i18n_lang.outInReport83_v1_2_8_0}}</span>
      <Select v-model="deduct_rebet" @on-change="getDeductRebet()" style="width: 100px;"  v-if="is_deduct">
        <Option :value="-1">{{i18n_lang.outInReport84_v1_2_8_0}}</Option>
        <Option :value="1">{{i18n_lang.outInReport85_v1_2_8_0}}</Option>
        <Option :value="0">{{i18n_lang.outInReport86_v1_2_8_0}}</Option>
      </Select>
      ...
      <Table
        style="margin-top:10px"
        border
        :columns="is_deduct ? columns.moneyColumns2 : columns.moneyColumns"
...
<script>
import { mainColumns, moneyColumns, moneyColumns2, totalColumns, backwaterColumns, channelList } from './columns'
...
export default {
  data() {
    ...
    return {
      ...
      columns: {
        ...
        moneyColumns2: moneyColumns2(this),
  ...
  watch: {
    ...
    //   this.endTimeOpt = {
    //     disabledDate: (date) => {
    //       return date && date.valueOf() < Date.now() - 604800000
    //     }
    //   }
    // }
  },
  methods: {
    ...
    // 获取出入款表数据
    async getData() {
      let number0 = new Date(this.query.start_time).getTime()
      let number1 = new Date(this.query.end_time).getTime()
      if(number0 - number1 >= 604800000 || number1 - number0 >= 604800000){
        return this.$msg({
          type: 'error',
          message: this.i18n_lang.outInReport46
        })
      }
      ...
      // }
      req.date_start = this.$dateTimeFormat(new Date(req.start_time))
      req.date_end = this.$dateTimeFormat(new Date(req.end_time))
      delete req.start_time
      delete req.timeArr
      delete req.end_time
      const res = await this.$axios.request({
        url: '/report/master/profit2',
        method: 'GET',
        params: req
      })
    ...
    async profitChannel(i = 1){
      ...
      req.start_time = this.$dateFormat(new Date(req.start_time))
      req.end_time = this.$dateFormat(new Date(req.end_time))
      delete req.timeArr
      const res = await this.$axios.request({
    ...
    async handleDetail1(type, name, is_deduct, deduct_rebet){
      this.isShowExport = false
      this.is_deduct = is_deduct
      this.deduct_rebet = deduct_rebet
      if(name == this.i18n_lang.outInReport64 || name == this.i18n_lang.outInReport80_v1_2_8_0){
        this.detail.query.deal_type = '701'
      }
      if(name == this.i18n_lang.outInReport65 || name == this.i18n_lang.outInReport81_v1_2_8_0){
        this.detail.query.deal_type = '702'
      }
      if(name == this.i18n_lang.outInReport66 || name == this.i18n_lang.outInReport82_v1_2_8_0){
        this.detail.query.deal_type = '703'
      }
      this.detail.query.name = name
      this.visible.modal = false
    ...
    async getDetail(i = 1) {
      ...
      const req = {
        ...
        deduct_rebet: this.deduct_rebet == -1 ? '' : this.deduct_rebet
      }
      ...
      req.start_time = this.$dateTimeFormat(new Date(this.query.start_time))
      req.end_time = this.$dateTimeFormat(new Date(this.query.end_time))
      delete req.timeArr
    ...
    handleTirdDetail(name) {
      this.$router.push({
        ...
        query: {
          username: name,
          // startTime: this.$dateTimeFormat(new Date(this.query.start_time)),
          ...
          startTime: formatDateToString(new Date(this.query.start_time)),
          endTime: formatDateToString(new Date(this.query.end_time))
        }
      })
    },
    // 查看用户详情
    ...
    async returnAmount(req){
      ...
      this.backwaterList.push({
        classify: `${this.i18n_lang.outInReport80_v1_2_8_0}`,
        people: res.data.day_deduct.people,
        amount: res.data.day_deduct.amount,
        num: res.data.day_deduct.num,
        is_deduct: 1
      })
      this.backwaterList.push({
      ...
      this.backwaterList.push({
        classify: `${this.i18n_lang.outInReport81_v1_2_8_0}`,
        people: res.data.week_deduct.people,
        amount: res.data.week_deduct.amount,
        num: res.data.week_deduct.num,
        is_deduct: 1
      })
      this.backwaterList.push({
      ...
      this.backwaterList.push({
        classify: `${this.i18n_lang.outInReport82_v1_2_8_0}`,
        people: res.data.month_deduct.people,
        amount: res.data.month_deduct.amount,
        num: res.data.month_deduct.num,
        is_deduct: 1
      })
      // this.backwaterList.push({
