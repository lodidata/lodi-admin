  ...
  <div class="Daily">
    ...
    <Card dis-hover class="query_box">
      <div class="item">
        ...
        ></DatePicker>
        <!-- <DatePicker v-model="searchInfo.date_start" type="date" placeholder style="width: 180px" :options="dateStartOpt" @on-change="selectStartTime" :editable="false"></DatePicker>
        <span>-</span>
        <DatePicker v-model="searchInfo.date_end" type="date" placeholder style="width: 180px" :options="dateEndOpt" @on-change="selectEndTime" :editable="false"></DatePicker> -->
...
<script>
  ...
  import { collectData, agentDetails, agentCntList } from './weekColumns'
  import MyTable from '_c/MyTable'
  const searchInfo = {
    model: 3,
    user_name: '',
    agent_name: '',
    // date_start: computeDay(-7),
    // date_end: computeDay()
    date_start: ``,
    date_end: ``,
    bet_switch: 0,
  ...
  export default {
    ...
    created() {
      ...
      // if(JSON.parse(parameter).DailySettlement1){
      //   this.searchInfo = {
      //     model: 1,
      //     user_name: JSON.parse(parameter).DailySettlement1.user_name, // 用户名
      //     agent_name: JSON.parse(parameter).DailySettlement1.agent_name,
      //     date_start: JSON.parse(parameter).DailySettlement1.date_start,
      //     date_end: JSON.parse(parameter).DailySettlement1.date_end,
      //   }
      //   this.page.currentPage = JSON.parse(parameter).DailySettlement1.page
      //   this.page.pageSize = JSON.parse(parameter).DailySettlement1.page_size
      //   this.reginTime = [this.searchInfo.date_start, this.searchInfo.date_end]
      //   this.getList(this.page.currentPage)
      // }else{
      //   this.getList()
      // }
      if(localStorage.getItem('check_/rebate/profitStatement')){
        let ps = localStorage.getItem('check_/rebate/profitStatement')
        if(ps.indexOf('yk') == -1 && ps.indexOf('bet') == -1){
          this.getTime()
        }
      }else{
        this.getTime()
    ...
    methods: {
      ...
      checkboxKeyFn(checkbox){
        if(checkbox.indexOf('yk') > -1 && this.searchInfo.loseearn_switch === 0){
          this.searchInfo.loseearn_switch = 1
          this.getList(this.page.currentPage)
        }
        if(checkbox.indexOf('yk') == -1 && this.searchInfo.loseearn_switch === 1){
          this.searchInfo.loseearn_switch = 0
          // this.getList(this.page.currentPage)
        }
        if(checkbox.indexOf('bet') > -1 && this.searchInfo.bet_switch === 0){
          this.searchInfo.bet_switch = 1
          this.getList(this.page.currentPage)
        }
        if(checkbox.indexOf('bet') == -1&& this.searchInfo.bet_switch === 1){
          this.searchInfo.bet_switch = 0
          // this.getList(this.page.currentPage)
        }
      },
      // 查看用户详情
      ...
      getTime(){
        ...
        // console.log('MondayTime', this.$dateFormat(new Date(MondayTime)))
        this.searchInfo.date_start = this.$dateFormat(new Date(MondayTime))
        this.searchInfo.date_end = this.$dateFormat(new Date(MondayTime))
        this.reginTime = [this.$dateFormat(new Date(MondayTime)), this.$dateFormat(new Date(MondayTime))]
        this.getList()
      },
      // 选择日期
      selectTime(val) {
      ...
      selectStartTime(val){
        ...
        this.dateEndOpt = {
          disabledDate: date => {
            return date.valueOf() <  getTime - getDay * 86400000 ||  date.valueOf() >  getTime + (7 - getDay) * 86400000
          }
        }
      },
      selectEndTime(val){
