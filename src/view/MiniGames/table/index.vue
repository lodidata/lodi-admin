<template>
  <div>
    ...
    <Card dis-hover class="query_box">
      <div class="query_con">
        <div class="item">
          <span>{{i18n_lang.electronicOrder0}}</span>
          <Input v-model="query.user_name" :placeholder="i18n_lang.electronicOrder1" style="width: 120px" />
        </div>
        <div class="item">
          <span>{{i18n_lang.electronicOrder2}}</span>
          <Input v-model="query.order_number" :placeholder="i18n_lang.electronicOrder3" style="width: 200px" />
        </div>
        <div class="item">
          <span>{{i18n_lang.electronicOrder4}}</span>
          <Select v-model="query.key" style="width:100px">
            <Option v-for="(game, i) in gameList" :key="i" :value="game.key">{{ game.name }}</Option>
          </Select>
        </div>
        <div class="item">
          <span>{{i18n_lang.electronicOrder5}}</span>
          <!-- <DatePicker
            ...
            :placeholder="i18n_lang.electronicOrder6"
            style="width: 200px"
          ></DatePicker> -->
          <DatePicker v-model="query.start_time" type="datetime" placeholder style="width: 180px" :options="startDateOpt" @on-change="selectStartTime" :editable="false"></DatePicker>
          <span>-</span>
          <DatePicker v-model="query.end_time" type="datetime" placeholder style="width: 180px" :options="endDateOpt" @on-change="selectEndTime" :editable="false"></DatePicker>
        </div>
      </div>
      <div class="query_btns">
        <Button type="primary" icon="ios-search" style="margin-right: 5px;" @click="getData()">{{i18n_lang.electronicOrder7}}</Button>
        <Button type="primary" icon="md-refresh" @click="reset">{{i18n_lang.electronicOrder8}}</Button>
      </div>
    </Card>
    ...
    <div style="padding:10px 0 0; font-size: 16px;text-align: end;">{{ `${i18n_lang.electronicOrder27}${page.total}${i18n_lang.electronicOrder28}` }}</div>
    <Page
      class="page_box"
      show-sizer
      split-panels
    ...
    <!-- 订单详情 -->
    <Modal v-model="visible" :title="i18n_lang.electronicOrder9" @on-visible-change="handleCloseModal">
      ...
      <div slot="footer">
        <Button type="primary" @click="visible = false">{{i18n_lang.electronicOrder10}}</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    pageTotal() {
      ...
      total.TableID = `${this.i18n_lang.electronicOrder11}${hash.length}${this.i18n_lang.electronicOrder12}`
      return total
    }
  },
  async created() {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).MiniGames){
        this.query = {
          user_name: JSON.parse(parameter).MiniGames.user_name ? JSON.parse(parameter).MiniGames.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).MiniGames.order_number ? JSON.parse(parameter).MiniGames.order_number : '', // 注单号
          category: JSON.parse(parameter).MiniGames.category ? JSON.parse(parameter).MiniGames.category : '', // 筛选列表
          key: JSON.parse(parameter).MiniGames.key ? JSON.parse(parameter).MiniGames.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).MiniGames.timeArr ? JSON.parse(parameter).MiniGames.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).MiniGames.start_time ? JSON.parse(parameter).MiniGames.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).MiniGames.end_time ? JSON.parse(parameter).MiniGames.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).MiniGames.orderby ? JSON.parse(parameter).MiniGames.orderby : '',
          orderbyasc: JSON.parse(parameter).MiniGames.orderbyasc ? JSON.parse(parameter).MiniGames.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).MiniGames.page)
      }else{
        this.getData()
      }
    },
    async getData(i = 1) {
      ...
      if (res.attributes) {
        ...
        this.statistics = {
          TableID: `${this.i18n_lang.electronicOrder13}${res.attributes.total_user_count}${this.i18n_lang.electronicOrder12}`,
