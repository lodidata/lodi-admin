  ...
  <div id="Money">
    ...
    <div class="content">
      <Row>
        ...
        <Col span="24">
          <div class="item">
            <p>{{i18n_lang.systemSet113_verification_code}}</p>
            <span>
              <Switch v-model="withdraw.withdraw_verify_switch"></Switch>
            </span>
          </div>
        </Col>
        <Col span="11">
        ...
        <Col span="11">
          <div class="item">
            <p>{{i18n_lang.systemSet109_v1_2_5_6}}</p>
            <span>
              <Input v-model="withdraw.withdraw_bkge_money.withdraw_min" style="width:80px"/>
              <span>-</span>
              <Input v-model="withdraw.withdraw_bkge_money.withdraw_max" style="width:80px"/>
            </span>
          </div>
        </Col>
      </Row>
...
export default {
  ...
  methods: {
    inputKeyUp(){
      this.withdraw.withdraw_day_times = this.withdraw.withdraw_day_times.replace(/\D/g,'')
    },
    keepInteger(){
    ...
    async getData() {
      ...
      withdraw.withdraw_money = {
        withdraw_max: MathDiv(withdraw.withdraw_money.withdraw_max, 100),
        withdraw_min: MathDiv(withdraw.withdraw_money.withdraw_min, 100)
      }
      withdraw.withdraw_card_money = {
        withdraw_max: MathDiv(withdraw.withdraw_card_money.withdraw_max, 100),
        withdraw_min: MathDiv(withdraw.withdraw_card_money.withdraw_min, 100)
      }
      withdraw.withdraw_bkge_money = {
        withdraw_max: MathDiv(withdraw.withdraw_bkge_money.withdraw_max, 100),
        withdraw_min: MathDiv(withdraw.withdraw_bkge_money.withdraw_min, 100)
      }
      recharge.recharge_money = {
        recharge_max: MathDiv(recharge.recharge_money.recharge_max, 100),
        recharge_min: MathDiv(recharge.recharge_money.recharge_min, 100)
      }
      if (recharge.recharge_money_value) {
        for (let i = 0; i < recharge.recharge_money_value.length; i++) {
          recharge.recharge_money_value[i] = MathDiv(recharge.recharge_money_value[i], 100)
        }
      }
    ...
    async save() {
      ...
      withdraw.withdraw_bkge_money = {
        withdraw_max: MathMul(withdraw.withdraw_bkge_money.withdraw_max, 100),
        withdraw_min: MathMul(withdraw.withdraw_bkge_money.withdraw_min, 100)
      }
      recharge.recharge_money = {
