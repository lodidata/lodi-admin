export default function(Vue) {
  ...
  return [
    ...
    },
    {
      ...
      minWidth: 100,
      render(h, { row }) {
        return (<span>{Number(row.bet) / 100}</span>)
      }
    },
    {
      ...
      minWidth: 100,
      render(h, { row }) {
        return (<span>{Number(row.send_prize) / 100}</span>)
      }
    },
    // {
    ...
    //   minWidth: 100,
    //   render(h, { row }) {
    //     return (<span>{Number(row.valid_bet) / 100}</span>)
    //   }
    // },
    {
      ...
      minWidth: 100,
      render(h, { row }) {
        return (<span>{Number(row.win_loss) / 100}</span>)
      }
    }
  ]
}
